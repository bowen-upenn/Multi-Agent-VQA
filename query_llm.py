...
class QueryLLM:
        ...
        if grader_id == 0:
            messages = [
                {"role": "system", "content": "Please grade the following answer provided by a large vision-language model (VLM) for a visual question answering task in one to two sentences. "
                              ...
                              "it should be graded as '[Grader 0] [Correct]'. If the VLM's answer contains the correct answer but has additional information not mentioned by the correct answer, it is still '[Grader 0] [Correct]'. " 
                              ...
                              "Partially correct is still '[Grader 0] [Correct]'. Otherwise, if the VLM's answer misses the targeted information, grade the answer as '[Grader 0] [Incorrect]'. Reason your grading step by step but keep it short. "},
            ...
            messages = [
                {"role": "system", "content": "Evaluate the accuracy of a VLM's response to a visual question. "
                                              ...
                                              "a response like 'The doll indeed has a bright color but it is not large' that correctly identifies at least one criterion not being met, even if other criteria are met, should be rated as '[Grader 1] [Correct]'. " 
                                              ...
                                              "Provide a brief rationale for your evaluation, highlighting the reasoning behind your decision. "},
            ...
            messages = [
                {"role": "system", "content": "Assess the response from a VLM on a visual question task. "
                                              ...
                                              "In cases where the question requires satisfying multiple criteria and the answer is negative, the response is '[Grader 2] [Correct]' if it correctly finds one criteria that is not met. "
                                              ...
                                              "Justify your assessment step by step but keep it brief. "},
    ...
    def query_llm(self, prompts, previous_response=None, target_answer=None, model_answer=None, grader_id=0, llm_model='gpt-3.5-turbo', step='related_objects', max_batch_size=4, verbose=False):
        # query on a single image
        if len(prompts) == 1:
            if llm_model == 'gpt-4':
                response = self._query_openai_gpt_4(prompts[0], step, previous_response=previous_response, target_answer=target_answer, model_answer=model_answer, grader_id=grader_id, verbose=verbose)
            else:
                response = self._query_openai_gpt_3p5(prompts[0], step, previous_response=previous_response, target_answer=target_answer, model_answer=model_answer, grader_id=grader_id, verbose=verbose)
            return response
        ...
        for start_idx in range(0, total_num_prompts, max_batch_size):
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_batch_size) as executor:
                if llm_model == 'gpt-4':
                    batch_responses = list(executor.map(lambda prompt: self._query_openai_gpt_4(prompt, step, previous_response=previous_response, target_answer=target_answer,
                                                                                                model_answer=model_answer, grader_id=grader_id, verbose=verbose), batch_prompts))
                else:
                    batch_responses = list(executor.map(lambda prompt: self._query_openai_gpt_3p5(prompt, step, previous_response=previous_response, target_answer=target_answer,
                                                                                                  model_answer=model_answer, grader_id=grader_id, verbose=verbose), batch_prompts))
    ...
    def _query_openai_gpt_3p5(self, prompt, step, previous_response=None, target_answer=None, model_answer=None, grader_id=0, verbose=False):
